...
const AddPost = ({ onSend }) => {
  ...
  useEffect(() => {
    if (location.state && location.state.image && location.state.defaultText) {
      setContent(location.state.defaultText);
      console.log("this" + location.state.image);
      fetch(location.state.image)
        .then((response) => response.blob())
        .then((blob) => {
          // Extract the file name from the path
          const fileName = location.state.image.split("/").pop();
          console.log(fileName); // Move the log statement here
          const file = new File([blob], fileName, { type: "image/png" });
          // Set the File object's path property with the value of the name property
          file.path = file.name;
          const fileInfo = `File Name: ${file.path}\nFile Type: ${file.type}\nFile Size: ${file.size} bytes`;

          setFiles([...files, file]);
        })
        .catch((error) => console.error("Error fetching badge image:", error));
    }
  }, [location.state]);
  ...
  const handleAddPost = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:5050/api/post/AddNewPost/${postData.UserID}`,
    ...
    <Stack direction="row" spacing={2} className={"AddPostcontainer"}>
      ...
      >
        ...
        <Stack
          direction="column"
          spacing={3}
          sx={{ height: "85vh", width: "95%" }}
          className={"Post-Info-Add"}
        >
          <Stack direction="column" spacing={0.5}>
            ...
            <Box>
              <div {...getRootProps()} className={`upload-container`}>
                ...
                />
                <p>
                  Drag & drop images here, or click to select images (Max 20
                  images)
                </p>
