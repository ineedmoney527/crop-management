...
const IrrigationPage = () => {
  ...
  const [wateringLevels, setWateringLevels] = useState({
    ...landInfo.wateringLevels,
  });
  const [nextWateringLevels, setNextWateringLevels] = useState({
    ...landInfo.nextPatch.wateringLevels,
  });
  // State to track checkbox statuses
  const [checkboxes, setCheckboxes] = useState(
    Array.from({ length: landInfo.wateringPoints }, (_, index) => false)
  );
  // Handler function to update checkboxes to all true
  const handleSelectAll = () => {
    setCheckboxes(Array.from({ length: landInfo.wateringPoints }, () => true));
  };
  const [nextPatchCheckboxes, setNextPatchCheckboxes] = useState(
    Array.from({ length: landInfo.wateringPoints }, () => false)
  );
  const handleSelectAllNextPatch = () => {
    setNextPatchCheckboxes(
      Array.from({ length: landInfo.wateringPoints }, () => true)
    );
  };
  ...
  return (
    <div className="irrigation-page">
            ...
            <Stack direction="row" spacing={1}>
              <div className="info-row">
                ...
                <span
                  ...
                  style={{
                    color: getHumidityColor(landInfo.soilHumidityLevel),
                  }}
                >
          ...
          <Stack direction="column">
            ...
            <div className="info-row">
              ...
              <label className="switch">
                <input
                  type="checkbox"
                  checked={landInfo.enabledStatus}
      ...
      <Divider>
        {" "}
        <Chip label="Adjustment Setting" size="small" />
      </Divider>
      ...
      <div className={"adjustContainer"}>
        <Stack direction="column" a spacing={5}>
          <Stack direction="row" spacing={1} className={"firstrow"}>
                ...
                >
                  ...
                  <Slider
                    ...
                    onChange={(event, newValue) =>
                      handleWaterLevelChange(`col${col}`, newValue)
                    }
            ...
            <Stack direction="column" spacing={1} className={"rd3col"}>
              <div className={"durationSetting"}>
                <h2 className="subTitle-irr">Set Duration</h2>
          ...
          <Stack direction="row" spacing={1} className={"sndrow"}>
            <Stack direction="column" spacing={1} className={"st1col"}>
              <Stack direction="row" spacing={1}>
                ...
                <label className="switch">
                  <input
                    type="checkbox"
                    checked={landInfo.nextPatch.status}
                    onChange={handleNextStatus}
                  />
                  <span className="slider round"></span>
                </label>
              </Stack>
              ...
              {[1, 2, 3, 4, 5].map((col) => (
                ...
                >
                  <Typography
                    ...
                    fontWeight="bold"
                  >
                    Column {col} Watering Level:{" "}
                    {nextWateringLevels[`col${col}`]}%
                  </Typography>
                  <Slider
                    min={0}
                    max={100}
                    ...
                    onChange={(event, newValue) =>
                      handleNextWaterLevelChange(`col${col}`, newValue)
                    }
                    aria-labelledby={`col-${col}-slider`}
                    disabled={
                      !landInfo.nextPatch.status || !landInfo.enabledStatus
                    }
                    className="watering-level-slider" // Apply slider styles
                  />
                </Box>
            ...
            <Stack direction="column" spacing={1} className={"rd3col"}>
              <div className={"durationSetting"}>
                <h2 className="subTitle-irr">Next Timer Setting</h2>
