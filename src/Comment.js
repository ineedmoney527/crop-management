...
function Comments({post}){
    ...
    const fetchComments = async () => {
        try {
            const response = await axios.get("http://localhost:5050/Comment/"+post)
    ...
    const handleAddComment = async() =>{
        ...
        try {
            ...
            const response = await axios.post(`http://localhost:5050/LeaveComment/${post}`, { content: content });
            fetchComments();
        ...
        <Box>
            ...
            >
                {comments?.length === 0 ? (
                    ...
                    >
                        <Typography variant="body1" sx={{ color: 'grey' }}>No Comments</Typography>
                    </Box>
                ) : (
                        ...
                        >
                            <Box sx={{ margin: "5px" }} justifyContent="flex-start">
                                <Box sx={{ display: "flex", flexDirection: "row", mb:'10px' }} >
                                    ...
                                    <Stack direction="column" spacing={0.3} className={"posterInfo-comment"}  >
                                        <label
                                            style={{
                                                wordWrap: "break-word",
                                        ...
                                        <label className={"postTime-comment"}>{moment(comment.CommentDate).fromNow()}</label>
                                    </Stack>
                                </Box>
                                <Typography
                                ...
                                <Divider variant="middle" sx={{width:'900px', backgroundColor:'#F9F9F9', mb:'5px', mt:'5px'}}/>
                            </Box>
                        </Box>
                    ))
                )}
            ...
            <Stack direction={"row"} sx={{width:'100%'}} alignItems="center">
                <form>
                    <TextareaAutosize
                        ...
                        style={{resize: 'none', width: '870px', height:'30px',
