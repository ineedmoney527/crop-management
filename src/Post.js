...
const Post = ({ post, onLike, onComment }) => {
  ...
  const fetchLikes = async () => {
    try {
      const response = await axios.get("http://localhost:5050/api/post/Likes");
  ...
  const fetchLikesCount = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5050/api/post/LikesCount/${post.PostID}`
      );
      // const likedPostIds = response.data.data.map((like) => like.PostID);
    ...
    try {
      if (isLiked) {
        const response = await axios.delete(
          `http://localhost:5050/api/post/Unlikes/${post.PostID}`
        );
        setIsLiked(false);
        fetchLikes();
        ...
        const response = await axios.put(
          `http://localhost:5050/api/post/SetLikes/${post.PostID}`
        );
        setIsLiked(true);
        fetchLikes();
        ...
        >
          ...
          >
            ...
            <Button
              ...
              startIcon={
                isLiked ? <GiHearts size={20} /> : <CiHeart size={25} />
              }
